...
public class G711DePacketizer {
     ...
     */
    public G711DePacketizer(int t140PayloadType, int redPt,
     ...
     */
    public synchronized int decode(RtpAudioBuffer inputBuffer,
				   RtpAudioBuffer outputBuffer) {
	...
	//byte[] data = filterZeros(bufferData); //ujwal:closed as mail by Andreas
	byte[] data = new byte[inputBuffer.getLength()];;
	System.arraycopy((byte[])inputBuffer.getData(),
			 inputBuffer.getOffset(),
			 data,
			 0,
	...
	/*if (redFlagIncoming) {
        ...
        //Check for sequencenumber wraparound
                 ...
                 AudioConstants.MAX_SEQUENCE_NUMBER-
                 AudioConstants.WRAP_AROUND_MARGIN &&
                 currentSequenceNumber <
                 AudioConstants.WRAP_AROUND_MARGIN) {

            //No packets lost
            if (lastSequenceNumber == AudioConstants.MAX_SEQUENCE_NUMBER &&
	...
	if (inputBuffer.getSsrc() != ssrc) {
        ...
        //New packet(s) missing. //closed as discussed with Andreas
			...
			new LossTimerTask(currentSequenceNumber-i, this);
                    if (redFlagIncoming) {
                        timer.schedule((TimerTask)ltt,
                              AudioConstants.WAIT_FOR_MISSING_PACKET_RED);
                    } else {
                        timer.schedule((TimerTask)ltt,
                                       AudioConstants.WAIT_FOR_MISSING_PACKET);
                    }
	...
	byte[] lastData = null;
		...
		if (!(lastData == AudioConstants.LOSS_CHAR &&
		      newData == AudioConstants.LOSS_CHAR)) {
    ...
    private byte[] getData(int generation,byte[] data) {
        ...
        for (int i=0;i<redundantGenerations;i++) {
	    blockLengthByteHigh =
		data[AudioConstants.REDUNDANT_HEADER_SIZE*i+2];
            blockLengthByteLow =
		data[AudioConstants.REDUNDANT_HEADER_SIZE*i+3];
        ...
        if (redundantGenerations>0) {
            startLength = redundantGenerations*
                AudioConstants.REDUNDANT_HEADER_SIZE+
                AudioConstants.PRIMARY_HEADER_SIZE;
        }
    ...
    */
    ...
    private class LossTimerTask extends TimerTask {
        ...
        private G711DePacketizer parent = null;
         ...
         */
        public LossTimerTask(long seq, G711DePacketizer parent) {
            sequenceNumber=seq;
            this.parent=parent;
        }
